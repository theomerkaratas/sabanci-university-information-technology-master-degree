...
function ensureFiles() {
    ...
    // Ensure config.json exists with default table count
    if (!fs.existsSync(CONFIG_FILE)) {
        fs.writeFileSync(CONFIG_FILE, JSON.stringify({ tableCount: 10 }, null, 2));
    } else {
        const config = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8'));
        if (config.tableCount === undefined) {
            config.tableCount = 10;
...
app.post('/api/tables/occupy', (req, res) => {
    ...
    // Validate table number against configured table count
    const config = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8'));
    if (table < 1 || table > config.tableCount) {
        return res.status(400).json({ message: `Invalid table number. Must be between 1 and ${config.tableCount}.` });
    }
...
app.get('/api/customers', (req, res) => {
    ...
    for (let i = 1; i < lines.length; i++) {
        const parts = lines[i].split(',').map(s => s.trim());
        const [u, p, t, at, pts] = parts;
        customers.push({ username: u, type: t, activeTable: at ? parseInt(at) : null, points: parseInt(pts) || 0 });
    }
    res.json(customers);
});
...
app.get('/api/config/tables', (req, res) => {
    ensureFiles();
    const config = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8'));
    res.json({ tableCount: config.tableCount });
});
...
app.put('/api/config/tables', (req, res) => {
    ...
    if (!Number.isInteger(tableCount) || tableCount < 1 || tableCount > 100) {
        return res.status(400).json({ message: 'Table count must be an integer between 1 and 100.' });
    }

    // Check if any customer is seated at a table beyond the new count
    const userContent = fs.readFileSync(USER_FILE, 'utf8');
    const userLines = userContent.split('\n').filter(l => l.trim());
    const conflicting = [];
    for (let i = 1; i < userLines.length; i++) {
        const parts = userLines[i].split(',').map(s => s.trim());
        const [u, p, t, at] = parts;
        if (at && parseInt(at) > tableCount) {
            conflicting.push({ username: u, table: parseInt(at) });
        }
    }
    if (conflicting.length > 0) {
        const names = conflicting.map(c => `${c.username} (Table ${c.table})`).join(', ');
        return res.status(400).json({ message: `Cannot reduce tables: ${names} are seated beyond table ${tableCount}.` });
    }
