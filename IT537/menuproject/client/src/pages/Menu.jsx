...
export default function Menu() {
  ...
  // Auto-show table selection modal on first login (no active table)
  useEffect(() => {
    if (user && user.type === 'customer' && !activeTable) {
  ...
  const handleTableSelect = async (tableNumber) => {
    try {
        ...
        showToast(`You are now seated at Table ${tableNumber}!`);
    } catch (error) {
  ...
  const handleLeaveTable = async () => {
    try {
        await leaveTable();
        showToast('You have left your table.');
    } catch (error) {
        showToast('Failed to leave table.');
    }
  };
